...
namespace KC
{
	...
	public:
                ...
                static std::string Get(void)
		{
            ...
            os  << pmc.WorkingSetSize / 1024 << "k / " << pmc.PeakWorkingSetSize / 1024 << "k + "
                << pmc.PagefileUsage / 1024 << "k / " << pmc.PeakPagefileUsage / 1024 << "k";
            return os.str();
		}
    #else           // linux环境
                static std::string Get(void)
		{
            return CProcMemInfo::get_proc_meminfo(getpid());
		}
        ...
        static std::string get_proc_meminfo(pid_t pid)
        {
            std::ostringstream os;
            procinfo info;
            if (get_proc_info(pid, &info) == 0)
                os  << info.rss * 4 << "k + " << info.vsize / 1024 << "k";
            return os.str();
        }
    #endif
	};
