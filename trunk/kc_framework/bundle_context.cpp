...
IBundle* CBundleContext::installBundle(const char* sName, const char* sPath, IBundle::TBundleState iAct)
{
	CBundle* result = nullptr;	try
	{
		...
		// 写安装成功日志
        bundle->WriteLogInfo((string(this->getHint("Install_Bundle_Success_")) + sName).c_str(), __FUNCTION__);
		result = bundle;
	}
...
bool CBundleContext::uninstallBundle(IBundle*& bundle)
{
	try
	{
		if (nullptr != bundle)
		{
			...
			{
				// 删除模块
                bundle->WriteLogInfo((this->getHint("Uninstall_Bundle_") + iter->first).c_str(), __FUNCTION__);
...
IServiceRegistration* CBundleContext::registerService(IService& srv)
{
	...
	try
	{
        ...
        ((IServiceEx&)srv).WriteLogInfo((this->getHint("Register_Service_Success_") + sName).c_str(), __FUNCTION__);
		// 所有的引用置为有效
...
{	bool bResult = false;
	try
	{
		...
		if (m_SrvRegMap.end() != iter)
		{
			...
			// 删除注册
            ((IServiceEx&)iter->second->getService()).WriteLogInfo((string(this->getHint("Unregister_Service_")) + iter->second->getService().getName()).c_str(), __FUNCTION__);
